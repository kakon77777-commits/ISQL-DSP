...
from .bridge import to_core_state_envelope
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="isql-dsr", description="ISQL Dynamic Spectrum Runtime v0.1")
    ...
    sp = sub.add_parser("bridge", help="Export a lossless ISQL Core STATE/R2 transport envelope")
    sp.add_argument("--state", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "hash":
            ...
            _emit({"schema": "isql.dsr-hash/v0.1", "identity": state.identity, "revision": state.revision, "state_hash": state_hash(state)})
            return 0
        ...
        if args.command == "apply":
            ...
            _emit({
                "schema": "isql.dsr-apply-result/v0.1",
        ...
        if args.command == "bridge":
            _emit(to_core_state_envelope(_read_state(args.state)).to_dict())
            return 0
